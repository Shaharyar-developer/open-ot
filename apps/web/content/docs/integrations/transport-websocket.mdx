...
const transport = new WebSocketTransport<Message>("ws://localhost:3000");
...
const transport = new WebSocketTransport<string>("ws://localhost:3000");
...
class ReconnectingTransport<M = unknown> {
  private transport: WebSocketTransport<M>;
  //        ^?
  private url: string;
  ...
  constructor(url: string, maxRetries = 5, retryDelay = 1000) {
    ...
    this.transport = new WebSocketTransport<M>(url);
  }
